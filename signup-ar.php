...
if(isset($_SESSION["username"])){
	header("location:http://www.youthfulamman.com/index-ar.php");
    exit();
}
?><?php
...
if(isset($_POST["usernamecheck"])){
    ...
    if (strlen($username) < 3 || strlen($username) > 16) {
	    echo '<strong style="color:#F00;">3 - 16 رمز وحرف</strong>';
	    exit();
    }
	if (is_numeric($username[0])) {
	    echo '<strong style="color:#F00;">يجب ان تبدأ بحرف</strong>';
	    exit();
    }
    if ($uname_check < 1) {
	    echo '<strong style="color:#009900;">' . $username . ' غير ماخةذ</strong>';
	    exit();
    } else {
	    echo '<strong style="color:#F00;">' . $username . ' ماخوذ</strong>';
	    exit();
    }
}
?><?php
...
if(isset($_POST["u"])){
	...
	if($u == "" || $e == "" || $p == "" || $g == "" || $c == ""){
        ...
        echo "اسم المستخدم المطلوب ناخوذ";
        exit();
	} else if ($e_check > 0){ 
        echo "البريد الالكتروني ماخوذ";
        exit();
	} else if (strlen($u) < 3 || strlen($u) > 16) {
        echo "اسم المستخدم يجب ان يكون بين 13 الى 16 حرف";
        exit(); 
    } else if (is_numeric($u[0])) {
        echo 'اسم يجب ان يبدا بحرف';
        exit();
    } else {
	// END FORM DATA ERROR HANDLING
		...
		$message = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>iBeez Message</title></head><body style="margin:0px; font-family:Tahoma, Geneva, sans-serif; text-align:right;"><div style="padding:10px; background:#333; font-size:24px; color:#CCC;"><a href="http://www.youthfulamman.com"><img src="http://www.youthfulamman.com/images/avatardefault.png" width="36" height="36" alt="iBeez" style="border:none; float:left; text-align:right;"></a>iBeez Account Activation</div><div style="padding:24px; font-size:17px; text-align:right;">مرحبا '.$u.',<br /><br />اضغط على الرابط في الاسفل لتفعيل حسابك<br /><br /><a href="http://www.youthfulamman.com/activation.php?id='.$uid.'&u='.$u.'&e='.$e.'&p='.$p_hash.'">اضغط هنا</a><br /><br />سجل الدخول بهذا البريد الالكتروني<br />* البريد <b>'.$e.'</b></div></body></html>';
		$headers = "From: $from\n";
...
function signup(){
	...
	var dod=_("dod").value;
	if(u == "" || e == "" || p1 == "" || p2 == "" || c == "" || g == ""){
		status.innerHTML = "الرجاء اكمال التعبءة";
	} else if(p1 != p2){
		status.innerHTML = "Your password fields do not match";
	} else if( _("terms").style.display == "none"){
		status.innerHTML = "الرجاء عرض شروط الاستخدام";
	} else {
		_("signupbtn").style.display = "none";
		status.innerHTML = 'الرجاء الانتظار ...';
		var ajax = ajaxObj("POST", "signup.php");
...
_("signupbtn").style.display = "block";
					...
					window.scrollTo(0,0);
					document.getElementById("signupform").style.textAlign="right";
					_("signupform").innerHTML = "حسناَ "+u+", تحقق بريدك الالكتروني <u>"+e+"</u> حتى تكمل اجرات التسجيل. لن تستطيع ان تشارك اذا لم تفعل حسابك.";
				}
	        }
        }
...
<div style="width:100%; height:100px;"> </div>
  ...
  <form style="width:300px; color:hsla(0,0%,0%,1.00);	margin-left:auto; margin-right:auto;" name="signupform" id="signupform" onsubmit="return false;">
    <div style="text-align:right;">اسم المستخدم</div>
    ...
    <div style="text-align:right;">كلمة المرور</div>
    ...
    <div style="text-align:right;">تاكيد كلمة المرور</div>
    ...
    <div style="text-align:right;">الجنس</div>
    <select style="border:none; width:300px;   padding:10px; border-radius:15px; background-color:hsla(0,0%,100%,1.00); color:hsla(0,0%,0%,1.00); border:none; " id="gender" onfocus="emptyElement('status')">
      ...
      <option style="text-align:right;" value="m">ذكر</option>
      <option style="text-align:right;" value="f">انثى</option>
    </select>
      <div style="text-align:right;">هل انت معلم؟</div>
    <select style="border:none; width:300px;   padding:10px; border-radius:15px; background-color:hsla(0,0%,100%,1.00); color:hsla(0,0%,0%,1.00); border:none;" id="dod" onfocus="">
      <option style="text-align:right;" value=""></option>
      <option style="text-align:right;" value="0">لا</option>
      <option style="text-align:right;" value="1">نعم</option>
    </select>
    <div style="text-align:right;">البلد</div>
...
<option value="Zimbabwe">Zimbabwe</option>
    ...
    <div style="text-align:right;">مجالك</div>
    ...
    <div style="text-align:right;">عنك</div>
    ...
    <div>
      <a href="#" style="text-align:right;" onclick="return false" onmousedown="openTerms()">
        عرض شروط الاستخدام
      </a>
    </div>
    <div style="border: none; text-align:right; color: #FFF; background-color: #4cc87f; margin-top:10px; border-radius: 40px; display:none;" id="terms" >
      <h3>شروط الاستخدام</h3>
      <p>1. من فضلك الا تكون عتصري هذه عبارة عن شبكة اجتماعية تم بناؤها ل مساعدة الناس.</p>
      <p>2. اي عمل ضد اي مستخدم سياخذ بعين الاعتبار.</p>
      <p>3. الرجاء الاستحمام قبل الدخول.</p>
      <p>4.استمتع.</p>
    </div>
    <br /><br />
       <span id="status"></span>
    <button style="border: none; color: #FFF; background-color: #4cc87f; padding: 125px; bottom: 0px; margin-top:10px; padding-top: 10px; padding-bottom: 20px; border-radius: 40px;"id="signupbtn" onclick="signup()">تسجيل</button>
